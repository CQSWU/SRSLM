...
Use a new result directory after changing an implementation, and avoid editing
an evaluation's source tree while it is running. These are reproduction
recommendations, not source-hash execution gates.
...
Users may supply their own compatible checkpoints. Recorded hashes describe the
files; they are not a prerequisite for running the code.
