...
class RePlanCore:
    ...
    def act(self, obs, skip_agents=None):
        ...
        for k in range(num_agents):
            ...
            self.planner[k].update_obstacles(
                obstacles,
                other_agents,
            ...
            if path is not None and path[1][0] < INF:
                action.append(
                    self.actions[(path[1][0] - path[0][0], path[1][1] - path[0][1])]
                )
...
class FixLoopsWrapper(NoPathSoRandomOrStayWrapper):
    def __init__(self, agent, stay_if_loop_prob=None, add_none_if_loop=False):
        ...
        self.stay_if_loop_prob = 0.5 if stay_if_loop_prob is None else stay_if_loop_prob
